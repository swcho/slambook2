...
for i, frame in enumerate(ds.frames[1:], start=1):
    curr = frame.left
    tracked = track_lk(prev, curr, seeds_prev, winSize=11,
                       maxLevel=3, maxIter=30, eps=0.01)
    ok = tracked[:, 2] > 0.5
    n = int(ok.sum())
    rate = n / seeds_prev.shape[0]
    dx = float((tracked[ok, 0] - seeds_prev[ok, 0]).mean()
               ) if n else float("nan")
    dy = float((tracked[ok, 1] - seeds_prev[ok, 1]).mean()
               ) if n else float("nan")
    print(
        f"frame {i - 1} → {i}: tracked {n}/{seeds_prev.shape[0]} ({rate * 100:.1f}%), mean (dx,dy)=({dx:+.2f},{dy:+.2f})")
...
for i, (rate, dx, dy) in enumerate(results):
    ...
    assert abs(
        dx) < 100, f"pair {i}→{i + 1}: |dx|={abs(dx):.1f} surprisingly large"
    assert abs(
        dy) < 30, f"pair {i}→{i + 1}: |dy|={abs(dy):.1f} surprisingly large"
...
seeds0 = detect(prev0, Detector.GFTT, maxFeatures=200,
                qualityLevel=0.01, minDistance=20)
...
# %%
print(
    f"OK — step07 temporal LK tracks {len(results)} consecutive pairs with rate ≥ 70 %")
