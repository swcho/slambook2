...
print(
    f"running VO on {len(ds.frames)} KITTI mini frames; baseline {cam1.baseline_m:.3f} m, fx {cam0.fx:.2f}")
...
# %%
frame0 = ds.frames[0]
seeds0 = detect(frame0.left, Detector.GFTT, maxFeatures=150,
...
valid0 = tri0[:, 4] > 0.5
# (M, 3) in frame-0 camera frame == world
landmarks_world = tri0[valid0, :3].copy()
# (M, 2) tracked location on frame 0 left
landmark_left_pts = seeds0[valid0][:, :2].copy()
...
T_cw_0 = np.eye(4)
smap.insert_keyframe(0, T_cw_0, set(
    lm_ids), Policy.CH13_DEFAULT, PolicyOptions(0.2))
...
for i, frame in enumerate(ds.frames[1:], start=1):
    ...
    tracked = track_lk(prev_left, curr_left, seeds,
    ...
    res = estimate_pose(pts3, pts2, K, init6, rounds=4,
                        iter_per_round=10, chi2_threshold=5.991)
    T_new = np.eye(4)
    T_new[:3, :] = res.T_cw
    trajectory.append(T_new)
    inliers = int(res.final_inlier_mask.sum())
    print(
        f"  frame {i}: tracked {n_tracked}/{prev_pts.shape[0]} ({rate * 100:.1f}%), PnP inliers {inliers}/{pts3.shape[0]}, t = {T_new[:3, 3]}")
    ...
    kf = decide_inlier_threshold(
        inliers, min_inliers=int(landmarks_world.shape[0] * 0.6))
    if kf.insert:
        print(f"    → insert KF (reason: {kf.reason})")
        smap.insert_keyframe(i, T_new, set(
            lm_ids[:inliers]), Policy.CH13_DEFAULT, PolicyOptions(0.2))
...
z_progression = ts[:, 2]
distances = [float(np.linalg.norm(ts[i] - ts[i - 1]))
...
assert all(
    d < 5.0 for d in distances), "an inter-frame jump > 5 m suggests divergence"
