...
masked = detect(frame0, Detector.GFTT, maxFeatures=200, qualityLevel=0.01, minDistance=20, blockSize=3, mask=mask)
...
print(f"GFTT + 100×100 mask hole → {masked.shape[0]} kps, {in_hole} inside hole")
...
seeds = detect(left0, Detector.GFTT, maxFeatures=200, qualityLevel=0.01, minDistance=20)

t_lk = time.perf_counter()
tracked = track_lk(left0, right0, seeds, winSize=11, maxLevel=3, maxIter=30, eps=0.01, useInitialFlow=False)
dt_lk = (time.perf_counter() - t_lk) * 1000

ok_mask = tracked[:, 2] > 0.5
ok_count = int(ok_mask.sum())
n = seeds.shape[0]
mean_dx = float((tracked[ok_mask, 0] - seeds[ok_mask, 0]).mean()) if ok_count else float("nan")
mean_dy = float((tracked[ok_mask, 1] - seeds[ok_mask, 1]).mean()) if ok_count else float("nan")
track_rate = ok_count / n if n else 0.0
print(f"LK left→right (no init) tracked {ok_count}/{n} ({track_rate * 100:.1f}%) in {dt_lk:.1f} ms")
...
mean_dx_init = float((tracked_init[ok_mask_init, 0] - seeds[ok_mask_init, 0]).mean()) if ok_init else float("nan")
print(f"LK with useInitialFlow tracked {ok_init}/{n} ({ok_init / n * 100:.1f}%), mean dx={mean_dx_init:+.2f}")
assert ok_init >= ok_count, f"useInitialFlow regressed: {ok_init} < {ok_count}"
assert abs(mean_dx_init + 22) < 1.5, f"useInitialFlow dx {mean_dx_init:.2f} not near -22"
